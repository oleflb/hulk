...
async fn spawn_all(ctx: Arc<Context>) -> Result<RunningStack> {
    ...
    join_set.spawn(kick::run_boxed(ctx.clone()));
    ...
    join_set.spawn(localization_recorder::run_boxed(ctx.clone()));
    join_set.spawn(look_around::run_boxed(ctx.clone()));
    join_set.spawn(look_at::run_boxed(ctx.clone()));
    join_set.spawn(low_command_publisher::run_boxed(ctx.clone()));
    ...
    join_set.spawn(walking::run_boxed(ctx.clone()));
