...
impl Widget for KeyPreview<'_> {
    fn ui(self, ui: &mut Ui) -> Response {
        let text = WidgetText::from(RichText::new(self.text).size(11.0));
        let button_padding = 2.0;
        let animation_shift_amount: f32 = 1.5;
        ...
        let painter = painter.with_clip_rect(painter.clip_rect().expand(4.0));
