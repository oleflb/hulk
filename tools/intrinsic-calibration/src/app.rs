...
impl CalibrationApp {
    ...
    fn update_match(&mut self, context: &egui::Context) {
        let Some(pair) = self.ros.latest_stereo_pair() else {
            self.status = "waiting for stereo image pair".to_string();
            return;
        };
        ...
        if self
            .current_match
            .as_ref()
            .is_some_and(|m| m.frame_identifier == pair.inner.frame_identifier)
        {
            return;
        }

        match self.load_match(context, pair.time, &pair.inner) {
            Ok(match_) => {
                self.status = format!(
...
impl App for CalibrationApp {
    fn update(&mut self, context: &egui::Context, _frame: &mut Frame) {
        ...
        if let Some((frame_identifier, image_size, left_tags, right_tags)) = capture_request {
            ...
            self.status = format!("captured {added} calibration sample(s)");
        }
    }
}
