...
use crate::marker::{MarkerType, create_marker_detector};
...
impl CalibrationApp {
    ...
    fn update_match(&mut self, context: &egui::Context) {
        ...
        if self
            .current_match
            .as_ref()
            .is_some_and(|m| m.frame_identifier == pair.value.frame_identifier)
        {
            return;
        }

        match self.load_match(context, pair.value.left.header.stamp.into(), &pair.value) {
            Ok(match_) => {
                self.status = format!(
                    "matched stereo pair, markers left: {}, right: {}",
...
impl App for CalibrationApp {
    fn update(&mut self, context: &egui::Context, _frame: &mut Frame) {
        ...
        let previous_marker_type = self.marker_type;
        let mut marker_type_changed = false;
        ...
        CentralPanel::default().show(context, |ui| {
            ...
            let common_tag_count = self.current_match.as_ref().map_or(0, |match_| {
                common_tags(&match_.left.tags, &match_.right.tags).len()
            });
            let controls = calibration_card(
                ui,
                &mut self.calibration,
                &mut self.marker_type,
                common_tag_count,
            );
            marker_type_changed = controls.marker_type_changed;
        ...
        if marker_type_changed {
            match create_marker_detector(self.marker_type) {
                Ok(detector) => {
                    self.detector = detector;
                    self.current_match = None;
                    self.calibration.clear_samples();
                    self.status = format!(
                        "marker type set to {}; samples cleared",
                        self.marker_type.label()
                    );
                }
                Err(error) => {
                    self.marker_type = previous_marker_type;
                    self.status = format!("failed to create marker detector: {error}");
                }
            }
