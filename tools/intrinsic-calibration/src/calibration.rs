...
const TAG_SIZE_METERS: f32 = 0.055;
...
fn calibrate(samples: &[CalibrationSample]) -> Result<CalibrationResult> {
    let image_size = image_size(samples)?;
    let object_points = object_points(samples.len());
...
fn calibrate_single(
    ...
    let mut dist_coeffs = zero_mat(1, 5)?;
...
fn object_points(count: usize) -> core::Vector<core::Vector<core::Point3f>> {
    let corners = core::Vector::from_slice(&[
        core::Point3f::new(0.0, 0.0, 0.0),
        core::Point3f::new(TAG_SIZE_METERS, 0.0, 0.0),
        core::Point3f::new(TAG_SIZE_METERS, TAG_SIZE_METERS, 0.0),
        core::Point3f::new(0.0, TAG_SIZE_METERS, 0.0),
    ]);
