use eframe::egui::{
    self, Align, Align2, Button, Color32, ComboBox, CornerRadius, DragValue, FontId, Grid,
    Image as EguiImage, Layout, Rect, RichText, Sense, Stroke, Ui, Vec2, load::SizedTexture,
};
use ros_z::time::Time;

use crate::calibration::{CalibrationResult, CalibrationSample, CalibrationState};
use crate::image::LoadedImage;
use crate::marker::{DetectedTag, MarkerType};

#[derive(Default)]
pub(crate) struct CalibrationCardResponse {
    pub(crate) capture_clicked: bool,
    pub(crate) marker_type_changed: bool,
}
...
pub(crate) fn calibration_card(
    ...
    marker_type: &mut MarkerType,
    common_tag_count: usize,
) -> CalibrationCardResponse {
    let mut response = CalibrationCardResponse::default();
    egui::Frame::group(ui.style())
        ...
        .show(ui, |ui| {
            ...
            ui.add_space(10.0);
            ui.horizontal(|ui| {
                ui.label(RichText::new("Marker type").color(Color32::from_rgb(148, 163, 184)));
                ComboBox::from_id_salt("marker-type")
                    .selected_text(marker_type.label())
                    .show_ui(ui, |ui| {
                        for candidate in MarkerType::ALL {
                            response.marker_type_changed |= ui
                                .selectable_value(marker_type, candidate, candidate.label())
                                .changed();
                        }
                    });

                ui.separator();
                ui.label(RichText::new("Marker size").color(Color32::from_rgb(148, 163, 184)));
                let mut marker_size_meters = calibration.marker_size_meters();
                if ui
                    .add(
                        DragValue::new(&mut marker_size_meters)
                            .range(0.001..=1.0)
                            .suffix(" m")
                            .speed(0.001),
                    )
                    .changed()
                {
                    calibration.set_marker_size_meters(marker_size_meters);
                }
            });
...
pub(crate) fn status_pill(ui: &mut Ui, status: &str) {
    let color = if status.starts_with("matched") || status.starts_with("captured") {
        Color32::from_rgb(34, 197, 94)
    } else if status.starts_with("failed") || status.contains("error") || status.contains("closed")
    {
...
fn calibration_result(ui: &mut Ui, result: &CalibrationResult) {
    Grid::new("calibration-result")
        ...
        .show(ui, |ui| {
            ...
            result_row(ui, "Distortion model", "rational 8 coeffs".to_string());
