...
use types::{field_dimensions::FieldDimensions, visual_odometry::TriangulatedFeature};
...
const FEATURE_CROSS_SIZE: f32 = 0.025;
const INITIAL_TRUNK_HEIGHT: f32 = 0.5;
const TORSO_FALLBACK_TRANSLATION: [f32; 3] = [0.0, 0.0, 0.6];
...
pub(crate) struct ViewerData {
    use_visual_odometry: bool,
    visual_odometer_pose: nalgebra::Isometry3<f32>,
    initial_camera_to_robot: Option<nalgebra::Isometry3<f32>>,
    triangulated_features: Vec<TriangulatedFeature>,
...
impl ViewerData {
    pub(crate) fn from_state(state: &ViewerState) -> Self {
        ...
        Self {
            use_visual_odometry: state.use_visual_odometry,
            visual_odometer_pose: state.visual_odometer_pose,
            initial_camera_to_robot: state.initial_camera_to_robot,
            triangulated_features: state.triangulated_features.clone(),
    ...
    fn camera_to_world(&self) -> nalgebra::Isometry3<f32> {
        let initial_camera_to_world = self.initial_camera_to_world();
        if self.use_visual_odometry {
            initial_camera_to_world * self.visual_odometer_pose
        } else {
            initial_camera_to_world
        }
    }

    fn robot_to_world(&self) -> nalgebra::Isometry3<f32> {
        if !self.use_visual_odometry {
            return self.initial_robot_to_world();
        }

        self.robot_to_camera()
            .map(|robot_to_camera| self.camera_to_world() * robot_to_camera)
            .unwrap_or_else(|| self.camera_to_world())
    }

    fn initial_camera_to_world(&self) -> nalgebra::Isometry3<f32> {
        self.initial_robot_to_world()
            * self
                .initial_camera_to_robot
                .or_else(|| self.camera_to_robot())
                .unwrap_or_else(nalgebra::Isometry3::identity)
    }

    fn initial_robot_to_world(&self) -> nalgebra::Isometry3<f32> {
        let torso_to_robot = self.torso_to_robot();
        let torso_translation = torso_to_robot.translation.vector;

        nalgebra::Isometry3::translation(
            -torso_translation.x,
            -torso_translation.y,
            INITIAL_TRUNK_HEIGHT - torso_translation.z,
        )
    }

    fn torso_to_robot(&self) -> nalgebra::Isometry3<f32> {
        self.robot_kinematics
            .as_ref()
            .map(|kinematics| kinematics.torso.torso_to_robot.inner)
            .unwrap_or_else(|| {
                nalgebra::Isometry3::translation(
                    TORSO_FALLBACK_TRANSLATION[0],
                    TORSO_FALLBACK_TRANSLATION[1],
                    TORSO_FALLBACK_TRANSLATION[2],
                )
            })
    }

    fn camera_to_robot(&self) -> Option<nalgebra::Isometry3<f32>> {
        self.robot_to_camera()
            .map(|robot_to_camera| robot_to_camera.inverse())
    }

    fn robot_to_camera(&self) -> Option<nalgebra::Isometry3<f32>> {
        self.camera_matrix
            .as_ref()
            .map(|camera_matrix| (camera_matrix.head_to_camera * camera_matrix.robot_to_head).inner)
    }
}
...
const LINK_DESCRIPTORS: &[LinkDescriptor] = &[
    LinkDescriptor {
        ...
        fallback_translation: TORSO_FALLBACK_TRANSLATION,
    },
    LinkDescriptor {
        name: "K1logo",
        mesh: "K1logo.STL",
...
fn setup_scene(
    ...
    let feature_material = materials.add(StandardMaterial {
    ...
    commands.spawn((
        ...
        Mesh3d(meshes.add(empty_mesh(PrimitiveTopology::LineList))),
        MeshMaterial3d(feature_material),
        Transform::default(),
        Visibility::Hidden,
    ));
...
fn update_camera_viewport(
    ...
    let transform = transform_from_isometry(data.camera_to_world());
    *frustum.1 = transform;
...
fn update_robot_links(
    ...
    let robot_to_world = data.robot_to_world();
    ...
    for (link, mut transform) in &mut links {
        ...
        *transform = transform_from_isometry(robot_to_world * link_to_robot);
    }
}

fn update_triangulated_features(
    data: Res<ViewerData>,
    mut features: Single<(&Mesh3d, &mut Visibility, &mut Transform), With<TriangulatedFeatures>>,
    mut meshes: ResMut<Assets<Mesh>>,
) {
    *features.2 = transform_from_isometry(data.camera_to_world());
    *features.1 = if data.triangulated_features.is_empty() {
        Visibility::Hidden
    } else {
        Visibility::Visible
    };
    ...
    meshes
        .insert(
            features.0.id(),
            triangulated_features_mesh(&data.triangulated_features),
        )
        .expect("triangulated features mesh handle should be valid");
}
...
fn triangulated_features_mesh(features: &[TriangulatedFeature]) -> Mesh {
    let mut positions = Vec::with_capacity(features.len() * 6);

    for feature in features {
        let point = [feature.x, feature.y, feature.z];
        for axis in 0..3 {
            let mut start = point;
            let mut end = point;
            start[axis] -= FEATURE_CROSS_SIZE;
            end[axis] += FEATURE_CROSS_SIZE;
            positions.push(camera_point(start));
            positions.push(camera_point(end));
        }
    }
