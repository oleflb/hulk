...
uv run export-xfeat-lighterglue-onnx ../../../etc/neural_networks/xfeat-lighterglue-128-subsampled.onnx --height 448 --width 544 --keypoints 128 --subsample
uv run export-xfeat-onnx ../../../etc/neural_networks/xfeat-512-subsampled.onnx --height 448 --width 544 --batch-size 2 --keypoints 512 --subsample --static
uv run export-xfeat-lighterglue-onnx ../../../etc/neural_networks/xfeat-stereo-lighterglue-192-subsampled.onnx --height 448 --width 544 --keypoints 192 --subsample --stereo-only
```
...
`export-xfeat-lighterglue-onnx` fuses current-frame extraction and matching for visual odometry. It takes two zero-copy NV12 inputs named `current_left` and `current_right`, each shaped `(height / 2, width / 2, 6)`, plus the previous-left feature state: `previous_left_keypoints` shaped `(keypoints, 2)`, `previous_left_descriptors` shaped `(keypoints, 64)`, and `previous_left_valid` shaped `(keypoints)`. It returns current-left/right keypoints, descriptors, valid masks, current-left-to-current-right stereo matches, previous-left-to-current-left temporal matches, and the reverse match directions for diagnostics. The Rust VO node only extracts the CPU-required outputs: current-left state, current-right keypoints, stereo matches, and temporal matches. Pass `--subsample` to run XFeat on the chroma-resolution NV12 image while preserving the full-resolution input contract. Pass `--stereo-only` to export only current-frame stereo matching; the Rust VO node then computes temporal descriptor matches on CPU. `--grid-rows` and `--grid-cols` optionally cap keypoints per spatial grid cell.
